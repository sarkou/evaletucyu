...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Regex pour l'e-mail
    $regex = '/^(?:[a-zA-Z]+-[a-zA-Z]+\.[a-zA-Z]+@etu\.(?:cyu|u-cergy)\.fr|[a-zA-Z]+\.[a-zA-Z]+@etu\.(?:cyu|u-cergy)\.fr)$/';

    // Vérifier si l'e-mail correspond à la regex
    if (preg_match($regex, $email)) {
        ...
        $message = [
            ...
            ',
            'TextPart' => 'Votre nouveau mot de passe est : ' . $motDePasseUnique . ' Connectez-vous ici : ' . $_SERVER['HTTP_HOST'] . $lienConnexion . ' Pour vous désabonner, visitez ' . $_SERVER['HTTP_HOST'] . $lienDesabonnement . '.',
            'Headers' => [
        ...
        if ($response->success()) {
            ...
            try {
                ...
                // Redirection vers la page de remerciement avec l'e-mail en paramètre
                header("Location: merci.php?email=" . urlencode($email));
        ...
        $erreur_index = 'L\'e-mail n\'est pas au format attendu.';
    }
}
?>
    ...
    <section class="container_index">
        <section class="carte_index">
            <form class="form_index" action="./index.php" method="post">
                ...
                <span>
                    <label for="">etudiant(e)</label>
                    <input type="radio" name="categorie" id="" value="etudiant" />
                    <label for="">personnel administratif</label>
                    <input type="radio" name="categorie" id="" value="administratif" />
                </span>
